...
后续如果 PSP、FC/NES 或其他平台要做内置模拟器，会放到对应目录：
...
FC/NES 从 v0.1.57 起按外部模拟器方案接入，不内置 FC/NES core；v0.1.58 增加 John NESS / Nes.emu 专用启动候选；v0.1.59 将显示名称编辑改为弹窗，并优化 FC/NES 外部 ROM 路径直启尝试；v0.1.60 将“移除封面”移动到顶部操作区，并从 FC/NES 推荐列表移除 John NESS。
...
扫描 ROM
↓
绑定一个外部 FC/NES 模拟器 App
↓
点击游戏
↓
通过 Android Intent 把 ROM 交给外部模拟器启动
```

这版先保证 FC/NES 平台能出现在启动器、能扫描、能管理、能交给外部 App。后续如果要做内置 FC/NES，再考虑接入 FCEUmm / Nestopia libretro core。
